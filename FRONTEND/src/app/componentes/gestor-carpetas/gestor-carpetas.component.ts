...
LinkImage:any;
  ...
  ngOnInit(): void {
    ...
    this.listarCarpetas(usuario.nickname.toString());
  ...
  listarCarpetas(nombre: string) {
    this.serviciolistarcarpeta.listarCarpeta(nombre).subscribe((res: any) => {
  ...
  obtenerCarpeta(nombre: any) {
    this.carpeta_actual.nombre = nombre;
    this.storage_user.setCarpeta(this.carpeta_actual);
  ...
  crearcarpeta() {
    this.serviciolistararchivo.crearCarpeta(this.nombrecarpeta).subscribe((res: any) => {
      this.ListaArchivos = res;
    });
    //titulo, texto, tipo, btnAceptxt, sn_Cancel, OneDialog, txtSecondDialog
